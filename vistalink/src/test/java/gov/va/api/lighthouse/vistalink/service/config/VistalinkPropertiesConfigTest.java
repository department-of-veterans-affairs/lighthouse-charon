...
public class VistalinkPropertiesConfigTest {
  ...
  void loadParsesVistalinkPropertiesFromFile() {
    ...
    assertThat(vp.getVistas().get(0)).isEqualTo(ConnectionDetails.builder()
        .host("dummyhost")
        .port(2222)
        .divisionIen("dummydivisionien")
        .build());
    assertThat(vp.getVistas().get(1)).isEqualTo(ConnectionDetails.builder()
        .host("testhost")
        .port(1111)
        .divisionIen("testdivisionien")
        .build());
    assertThat(vp.getVistas().get(2)).isEqualTo(ConnectionDetails.builder()
        .host("fakehost")
        .port(3333)
